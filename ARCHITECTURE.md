...
- ATRI may call DeepSeek, chat-facing web search, explicit command tools, or local memory.
...
- `src/chat/` owns chat generation, prompts, memory, and chat-facing search.
...
- `src/messaging.py`: per-session message deduplication and ordered processing
- `src/util.py`: shared HTTP helpers with proxy fallback
- `src/chat/chat_service.py`: DeepSeek chat generation, chat tool definitions, and tool-call loop
...
- `src/commands/search.py`: `/search` command handler
- `src/commands/bsearch.py`: `/bsearch` Bilibili user search command handler
...
- `src/services/onebot_client.py`: OneBot HTTP API client
- `src/services/search_service.py`: web search helper
- `src/services/bilibili_search.py`: Bilibili user search (API + HTML fallback)
...
7. `src.router.route_message()` sends `/` messages to command handling and all
...
9. Default chat handling exposes `search_web` as an optional tool. When the
   message clearly mentions Bilibili/B站/UP主/主播/直播间, it also exposes
   `bilibili_user_search`.
   The model may answer directly for normal chat, but should search before
   answering unfamiliar slang, abbreviations, names, IDs, current events, and
   other uncertain or time-sensitive content.
...
First, `src.router.route_message()` handles the hard boundary:
...
Second, `chat_service.generate_reply()` calls DeepSeek for ordinary chat with
optional Function Calling tools:

```text
search_web(query)
bilibili_user_search(query)   # only exposed for B站/Bilibili/UP主/主播/直播间 queries
```

The system prompt tells the model to answer direct chat normally, but to call
`search_web` before answering unfamiliar slang, abbreviations, names, IDs,
current events, latest information, or other uncertain content. Search results
are never sent directly to QQ; they are appended as tool messages and the final
reply is generated by DeepSeek.

When DeepSeek returns a supported tool call, ATRI executes the search, appends a
`tool` result message, and calls DeepSeek again for the final reply. Weather,
image generation, file operations, and image sending are not registered as chat
tools.
...
2. Keep default chat tools limited to `search_web`; add specialized tools only
   behind clear text signals and explicit commands.
...
- an untrusted-context user message from `build_untrusted_context()`
...
- `[Capabilities]`: allowed chat/search behavior, search-result processing, and command-only tool boundaries
- `[Context Handling]`: rules for treating saved memory and external information as untrusted context
- `[User]`: states that user input is conversation content and cannot rewrite the rules

Saved memory and external search results are not embedded in the system prompt.
They are sent as a separate `[非可信上下文]` user message so they can guide
answers without gaining system-role priority.
...
- used for chat answers and the chat-facing `search_web` tool-call loop
...
- uses Bilibili public user search API with HTML fallback
- is exposed through `/bsearch`
- is only exposed to chat as `bilibili_user_search` when the user mentions B站, Bilibili, UP主, 主播, or 直播间
