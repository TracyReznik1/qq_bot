...
- Windows 上“停止 Bot”会立即结束管理器启动的 Bot 核心进程；其他平台保留正常退出和超时强制结束机制。
- “重启 Bot”只重启 Bot 核心，不会重启 NapCat 或改变 QQ 登录状态；替代进程成功运行后会更新状态并弹出结果提示。
- QQ 扫码登录会缓存 NapCat 提前输出的 WebUI 信息，扫码成功后更新状态并弹出成功提示。
- “退出 QQ 并停止 NapCat”用于切换账号：先通过 NapCat WebUI 清除快速登录账号，再尝试 `/bot_exit` 并停止管理器内部启动的 NapCat。`/bot_exit` 断开连接不作为失败依据；外部实例不会按进程名强制结束。
