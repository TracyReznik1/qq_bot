...
- `src/chat/search_tool.py`: the only tool default chat may receive, `search_web`
...
9. Default chat handling always exposes only `search_web` as an optional tool.
...
one optional Function Calling tool:
...
2. Keep default chat tools limited to `search_web`.
