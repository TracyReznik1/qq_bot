...
> 基于 OneBot 协议的轻量级 QQ 聊天机器人，接入 Gemini 与 DeepSeek 大模型。支持上下文对话、网页 URL 自动前置直读、B站视频/字幕极速总结、确定性证据化联网检索、多模态视觉理解与细粒度结构化记忆，并内置三层 Prompt 注入防御机制。
...
- **📺 B站视频总结与 UP主名片查询**：原生轻量级提取 B 站视频元数据与 CC/AI 字幕文本，支持 `/video <链接/BV>`（别名 `/v`、`/bv`）与聊天链接前置直读；支持 `/up <名称或UID>`（别名 `/biliup`）瞬间生成结构化纯文本名片并展示最新投稿，支持结合提问调用模型深度分析。采用官方 WBI 鉴权，零无头浏览器/Playwright 依赖。
- **🔍 确定性联网检索**：Tavily 是主搜索提供者，遇到异常平滑回退至 DDGS（DDGS 的阶段超时默认为 15 秒）。普通聊天始终固定为由轻量检索路由（SearchRouter，基于检索收益判定）在免搜索与 `LIGHT 模式`（单查询快速检索，普通回复不展示任何来源编号、标题或 URL）之间动态裁决，闲聊或自包含任务直接免搜索，需要检索时跳过二次规划极速作答；显式 `/search` 为 `STANDARD 模式`（多查询，附带来源），支持 `/skip` 跳过搜索。遇到网络不可用、证据不足、名称/前提不一致时清晰说明，固定边界降级，确保基于已有可用证据生成，不伪造在线证据；遇到服务拒绝有效范围参数时移除日期过滤后重试一次 Tavily；安全 Trace 全程记录但剔除敏感正文。
- **🛡️ 三层 Prompt 注入防御**：
...
- **清晰边界**：保持实用窄边界，不支持通用视频流画面分析、天气或外部重量级爬虫/复杂 Agent。`/search <关键词>` 专注于关键词证据化检索（不提供独立 URL 直读）；聊天中的网页/视频直读由系统自动安全处理。
...
非 `/` 开头的消息直接进入日常对话。若消息中包含 HTTP/HTTPS 链接或 B 站视频（链接、短链或 BV/av 号），会自动前置直读网页全文或视频字幕并沙箱总结，短路跳过搜索；普通消息由轻量检索路由（SearchRouter，默认 `gemini-3.1-flash-lite`）基于检索收益动态裁决（闲聊/自包含任务免搜索，事实/时效动态以 LIGHT 模式快速检索回答并跳过二次规划）；显式 `/search` 强制 STANDARD 权威搜索，`/skip` 严格跳过搜索。

| 指令 | 别名 | 功能说明 |
|---|---|---|
| `/search <关键词>` | `/s <关键词>` | 显式强制联网搜索（STANDARD 模式），生成多查询并展示来源链接。 |
| `/skip [问题]` | 无 | 跳过联网搜索（SKIP 模式），直接由模型回答，支持附带图片，零搜索耗时与 Trace。 |
| `/video <链接或BV号>` | `/v`, `/bv` | 提取 B 站视频信息与 CC/AI 字幕并生成结构化总结，短路联网搜索。 |
| `/up <昵称或UID>` | `/biliup` | 查询 B 站 UP主名片、粉丝数、认证与最新投稿视频，支持附加提问深度分析。 |
