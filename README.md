...
- **📺 B站视频极速总结**：原生轻量级提取 B 站视频元数据与 CC/AI 字幕文本，支持显式 `/video <链接/BV>`（别名 `/v`、`/bv`）指令以及聊天中发送 B 站链接（`bilibili.com/video/BV...`、`b23.tv/...` 或直接输入 BV/av 号）自动直读总结并短路搜索。采用纯原生 HTTP API，零无头浏览器/Playwright 依赖，字幕最长安全截断 20,000 字符并严密转义注入 XML 沙箱。
